...
function init() {
    ...
    loader.load('models/huoyao.glb', (loadedGltf) => {
        // 自动居中模型
        ...
        loadedGltf.scene.traverse((object) => {
            if (object.isMesh) {
                // 使用更深的棕色（原色为0x8B4513）
                object.material.color.set(0x2F1B0A); // 深棕黑色

                // 调整材质属性增加金属质感
                object.material.metalness = 0.6;    // 金属度（0-1）
                object.material.roughness = 0.7;   // 粗糙度（0-1）

                // 降低自发光强度
                object.material.emissive.set(0x080402);
